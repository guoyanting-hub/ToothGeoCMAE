...
def load_mesh_seg(path, normalize=True, augments=[], request=[], patch_size=64):
    ...
    with open(label_path) as f:
        segment = json.load(f)
    faces_labels = np.array(segment['labels']) #- 1
    ...
    return faces_patcha, feats_patcha, Fs_patcha, centers_patcha, cordinates_patcha, label_patcha, face_adj_fixed, sample_name
...
class SegmentationDataset(data.Dataset):
    def __init__(self, dataroot, train=True, augments=None, finetune_list=None):
        super().__init__()
    ...
    def browse_dataroot(self):
        ...
        if subset_root == 'train':
            # 使用部分标注数据进行微调
            # list_txt = dataroot.parent / "finetune_84.txt"
            if self.finetune_list is not None:
                list_txt = Path(self.finetune_list)
            else:
                list_txt = dataroot.parent / "finetune_120.txt"  # fallback（可保留）
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            faces_patcha, feats_patcha, Fs_patcha, center_patcha, cordinates_patcha, label_patcha, face_adj, sample_name = load_mesh_seg(self.mesh_paths[idx], normalize=True, augments=self.augments, request=self.feats)
            return faces_patcha, feats_patcha, Fs_patcha, center_patcha, cordinates_patcha, label_patcha, face_adj, sample_name
        else:
            faces_patcha, feats_patcha, Fs_patcha, center_patcha, cordinates_patcha, label_patcha, face_adj, sample_name = load_mesh_seg(self.mesh_paths[idx], normalize=True, request=self.feats)
