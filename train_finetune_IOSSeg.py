...
import random
from model.dataset_IOSSeg import SegmentationDataset
...
def setup_logger(args, name, finetune_dataset_len, test_dataset_len):
    # # 修改日志目录结构
    log_dir = os.path.join('logs_IOSSeg', name)
...
def test(net, criterion, test_dataset, epoch, args, checkpoint_path):
    net.eval()
    running_loss = 0.0
    running_corrects = 0
    n_samples = 0
    ...
    for i, (face_patch, feats_patch, np_Fs, center_patch, coordinate_patch, labels, face_adj, sample_names) in enumerate(test_dataset):
        # ---------- 数据搬到 GPU ----------
    ...
    if test.best_acc < epoch_acc:
        ...
        # torch.save(copy.deepcopy(net.state_dict()), os.path.join('checkpoints_IOSSeg', run_name, 'best.pkl'))
        torch.save(copy.deepcopy(net.state_dict()), os.path.join(checkpoint_path, 'best.pkl'))
    ...
    # ================== 保存日志 ==================
    # testing_log.txt
    os.makedirs(os.path.join('logs_IOSSeg', run_name), exist_ok=True)
    with open(os.path.join('logs_IOSSeg', run_name, 'testing_log.txt'), 'a') as f:
...
def train(net, optim, criterion, finetune_dataset, epoch, args):
    ...
    for i, (faces_patch, feats_patch, np_Fs, centers_patch, coordinates_patch, labels, face_adj, sample_names) in enumerate(finetune_dataset):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--finetune_list', type=str, default=None, help='Path to finetune txt file (e.g., finetune_84.txt)')
    ...
    # ========== Dataset ==========
    finetune_dataset = SegmentationDataset(finetune_dataroot, train=True, augments=augments, finetune_list=args.finetune_list)
    ...
    checkpoint_path = os.path.join('checkpoints_IOSSeg', run_name)
    ...
    if args.mode == 'train':
        for epoch in range(args.n_epoch):
            ...
            if epoch % 10 == 0:
                test(net, criterion, test_data_loader, epoch, args, checkpoint_path)
                print('test finished')
            scheduler.step()
